...
public class TestTraderStrategy {
    ...
    public void testExecuteStrategy() {
        ...
        doNothing().when(traderFacade).placeOrder(any(Order.Action.class), Order.Type.LIMIT, anyString(), anyInt(), anyDouble());

        traderStrategy.executeStrategy();
        verify(traderFacade, times(1)).placeOrder(any(Order.Action.class), Order.Type.LIMIT, anyString(), anyLong(), anyDouble());
    }
